...
import type { PropFirmName, Trade, TradeStrategy } from "@/lib/types";
import { accountPhases, brokerChoices, directions, propFirms, results, sessions, strategies, tradingAreas } from "@/lib/types";

const propFirmRules: Record<PropFirmName, string[]> = {
  None: ["No prop firm selected. Use this for personal accounts or pure forward testing."],
  FTMO: ["Common rules: daily loss limit, max overall loss, profit target, minimum trading days, and no prohibited trading behavior."],
  The5ers: ["Common rules: max drawdown, daily pause discipline, consistency, and news/weekend rules depending on program."],
  "Funding Pips": ["Common rules: daily drawdown, overall drawdown, profit target, minimum days, and consistency controls."],
  FundedNext: ["Common rules: daily loss, overall loss, profit split phase, consistency, and news restrictions by account type."],
  "Alpha Capital": ["Common rules: max daily loss, max loss, target, minimum trading days, and risk limits."],
  MyFundedFX: ["Common rules: max daily drawdown, max total drawdown, target, and payout/account scaling conditions."],
};

const brokerNotes: Record<string, string[]> = {
  None: ["Broker not selected. Never store broker password or login credentials."],
  "IC Markets": ["Track spread, commission, slippage, server time, symbol suffix, leverage, and execution quality."],
  Pepperstone: ["Track spread, commission, slippage, platform type, leverage, and rollover/session execution."],
  OANDA: ["Track spread model, unit sizing, margin, execution, and session pricing behavior."],
  Eightcap: ["Track account currency, commission, spread, leverage, and symbol contract details."],
  XM: ["Track spread type, leverage, swap settings, bonus impact, and execution quality."],
  Exness: ["Track leverage mode, spread, commission if any, swap settings, and execution stability."],
  Deriv: ["Track synthetic/forex account type, contract specs, spreads, and session behavior."],
};
...
export function TradeForm({
  ...
  const selectedFirmRules = propFirmRules[trade.propFirmName ?? "None"] ?? propFirmRules.None;
  const selectedBrokerNotes = brokerNotes[trade.brokerName ?? "None"] ?? brokerNotes.None;
  ...
  const update = <K extends keyof Trade>(key: K, value: Trade[K]) => {
    setTrade((current) => {
      ...
      if (key === "area" && value === "Backtesting") {
        next.propFirmName = "None";
        next.accountSize = 0;
        next.accountPhase = "Evaluation";
        next.brokerName = "None";
      }
        ...
        >
          ...
          {needsAccountProfile ? (
            <div className="grid gap-4 rounded-lg border bg-background/45 p-4">
              <div>
                <h3 className="text-sm font-semibold">Company, account, and broker profile</h3>
                <p className="mt-1 text-xs leading-5 text-muted-foreground">
                  Forward/Funded/Account trades must be tied to the company/account profile you are trading. Backtesting stays independent from this data.
                </p>
              </div>
              <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                <Field label="Prop firm / company">
                  <Select value={trade.propFirmName ?? "None"} onChange={(event) => update("propFirmName", event.target.value as Trade["propFirmName"])}>
                    {propFirms.map((firm) => (
                      <option key={firm}>{firm}</option>
                    ))}
                  </Select>
                </Field>
                <NumberField label="Account size" value={trade.accountSize ?? 0} onChange={(value) => update("accountSize", value)} />
                <Field label="Account phase / level">
                  <Select value={trade.accountPhase ?? "Evaluation"} onChange={(event) => update("accountPhase", event.target.value as Trade["accountPhase"])}>
                ...
                <Field label="Broker">
                  <Select value={trade.brokerName ?? "None"} onChange={(event) => update("brokerName", event.target.value as Trade["brokerName"])}>
              ...
              <div className="grid gap-3 lg:grid-cols-2">
                <div className="rounded-md border bg-background/60 p-3">
                  <p className="text-xs font-semibold uppercase text-muted-foreground">Company rules checklist</p>
        ...
        <div className="fixed inset-0 z-[70] grid place-items-center bg-black/65 p-4 backdrop-blur-sm">
          <div className="glass-panel max-h-[88vh] w-full max-w-3xl overflow-auto rounded-lg border bg-background/95 p-5 shadow-2xl">
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
              ...
              {reviewTrade.area !== "Backtesting" ? <ReviewItem label="Company" value={reviewTrade.propFirmName ?? "None"} /> : null}
              ...
              {reviewTrade.area !== "Backtesting" ? <ReviewItem label="Broker" value={reviewTrade.brokerName ?? "None"} /> : null}
