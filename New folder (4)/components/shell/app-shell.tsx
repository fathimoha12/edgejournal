...
export function AppShell({
  ...
  async function handleLogout() {
    ...
    window.localStorage.removeItem("gold-uleeye-session");
    router.push("/login");
  }
    ...
    <div className="min-h-screen bg-background text-foreground">
      <aside className="fixed inset-y-0 left-0 z-30 hidden w-72 border-r bg-card/70 p-4 backdrop-blur-xl lg:block">
        <Link href="/" className="mb-8 flex items-center gap-3 rounded-md px-2 py-2">
          <BrandMark />
          <div>
            <p className="font-semibold tracking-tight">Gold Uleeye</p>
            <p className="text-xs text-muted-foreground">Forex journal OS</p>
          </div>
        </Link>
        ...
        <div className="absolute inset-x-4 bottom-4 grid gap-3 rounded-lg border bg-background/60 p-4">
          <div className="mb-3 flex items-center gap-2 text-sm font-medium">
            ...
            Journal-only security
          </div>
          <p className="text-xs leading-5 text-muted-foreground">
            No broker credentials are stored. Supabase RLS keeps every trader&apos;s data scoped to their account.
          </p>
