...
export function AppearanceSettings() {
  ...
  React.useEffect(() => {
    const saved = window.localStorage.getItem("gold-uleeye-accent-color");
    if (saved) setColor(saved);
  }, []);

  const setAccent = (nextColor: string) => {
    setColor(nextColor);
    window.localStorage.setItem("gold-uleeye-accent-color", nextColor);
  ...
  const uploadLogo = (file?: File) => {
    ...
    reader.onload = () => {
      window.localStorage.setItem("gold-uleeye-logo", String(reader.result));
      window.dispatchEvent(new Event("gold-uleeye-logo"));
            ...
            <div className="grid content-center gap-2">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <Button
                  ...
                  className="w-fit"
                  onClick={() => {
                    window.localStorage.removeItem("gold-uleeye-logo");
                    window.dispatchEvent(new Event("gold-uleeye-logo"));
                    setLogoName("");
                  }}
                >
              ...
              <p className="text-xs text-muted-foreground">{logoName || "Logo-ga wuxuu ka muuqanayaa meesha Gold Uleeye ku qoran tahay."}</p>
            </div>
          </div>
        </div>
      </CardContent>
